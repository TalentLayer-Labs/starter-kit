...
function EditTrustScore() {
  ...
  const formatClaims = (claims: any) => {
    ...
    claims.map((claim: any) => {
      // format dates
      if (claim.criteria === "accountCreation") {
  ...
  // Define an interface for the claim object
  interface Claim {
    criteria: string;
    value: string | number;
  }
  ...
  const TrustScore = (): JSX.Element => {
    if (!decryptedDatas) {
      return <div>No decrypted data</div>;
    }
    ...
    // Filter out the criteria we don't use and handle potential date for totalCommits
    const filteredData = (Array.isArray(decryptedDatas) ? decryptedDatas : [])
      .filter((claim: Claim) => {
        return ['totalStars', 'totalPRsMerged', 'totalCommits', 'followers', 'accountCreation'].includes(claim.criteria);
      })
      .map((claim: Claim) => {
        if (claim.criteria === 'totalCommits' && moment(claim.value, moment.ISO_8601, true).isValid()) {
          // If totalCommits is a valid date, treat it as account creation date
          return { ...claim, criteria: 'accountCreation' };
        }
        return claim;
      });
    ...
    // Normalize matrix points to a value between 0 and maxValue
    ...
    return (
      <div className="text-gray-800 flex justify-center items-center bg-white p-2 rounded-xl w-1/2 m-auto">
        ...
        <p className="ms-2 text-sm font-bold text-gray-500">{finalTrustScore.toFixed(2)}</p>
