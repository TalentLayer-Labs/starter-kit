...
function DelegateModal() {
  ...
  const onSubmit = async (validateState: boolean) => {
    ...
    await toggleDelegation(chainId, user.id, delegateAddress, publicClient, walletClient, validateState);
            ...
            <div className='flex items-center p-6 space-x-2 rounded-b border-t border-redpraha '>
              {hasPlatformAsDelegate ? (
                <button
                  ...
                  type='button'
                  className='hover:text-blue-600 hover:bg-red-50 bg-red-500 text-white rounded-xl px-5 py-2.5 text-center'>
                  Cancel Delegation
                </button>
              ) : (
                <button
