          ...
          <div
            className={`py-3 px-4 text-sm ${
              isSender && message.status === ChatMessageStatus.SENT
                ? 'ml-12 bg-info text-base rounded-bl-2xl rounded-tl-2xl rounded-tr-xl'
                ...
                ? 'ml-12 bg-base-200 text-base rounded-bl-2xl rounded-tl-2xl rounded-tr-xl'
                : 'mr-12 bg-base-200 text-base rounded-br-2xl rounded-tr-2xl rounded-tl-xl'
            }
          text-base`}>
            <span className='pr-1 text-base opacity-50 text-xs w-[50px]'>
...
const DateDivider = ({ date }: { date?: Date }): JSX.Element => (
  <div className='flex items-center pb-8 pt-4'>
    ...
    <span className='mx-11 flex-none text-base text-sm font-semibold'>
